...
import embedded_postgres.utils
import socket
from embedded_postgres.utils import find_suitable_port, process_is_running
import psutil
import platform
...
def _check_sqlalchemy_works(srv : embedded_postgres.PostgresServer):
...
def _check_postmaster_info(pgdata : Path, postmaster_info : embedded_postgres.utils.PostmasterInfo):
...
def _check_server(pg : embedded_postgres.PostgresServer) -> int:
    assert pg.pgdata.exists()
    postmaster_info = embedded_postgres.utils.PostmasterInfo.read_from_pgdata(pg.pgdata)
    assert postmaster_info is not None
...
def test_get_server():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            with embedded_postgres.get_server(tmpdir) as pg:
...
def test_reentrant():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            with embedded_postgres.get_server(tmpdir) as pg:
                pid = _check_server(pg)
                with embedded_postgres.get_server(tmpdir) as pg2:
                    assert pg2 is pg
                    _check_server(pg)
...
def _start_server_in_separate_process(pgdata, queue_in : Optional[mp.Queue], queue_out : mp.Queue, cleanup_mode : Optional[str]):
    with embedded_postgres.get_server(pgdata, cleanup_mode=cleanup_mode) as pg:
        pid = _check_server(pg)
        queue_out.put(pid)
...
def test_pg_ctl():
    if platform.system() != 'Windows' and os.geteuid() == 0:
        ...
        # embedded_postgres does this internally, but not worth it for this test
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            with embedded_postgres.get_server(tmpdir) as pg:
                output = embedded_postgres.pg_ctl(['status'], str(pg.pgdata))
...
def test_stale_postmaster():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            with embedded_postgres.get_server(tmpdir, cleanup_mode='stop') as pg:
                pid = _check_server(pg)
                pgdata = pg.pgdata
...
def test_cleanup_delete():
    with tempfile.TemporaryDirectory() as tmpdir:
        pid = None
        try:
            with embedded_postgres.get_server(tmpdir, cleanup_mode='delete') as pg:
                pid = _check_server(pg)
...
def test_cleanup_none():
    with tempfile.TemporaryDirectory() as tmpdir:
        pid = None
        try:
            with embedded_postgres.get_server(tmpdir, cleanup_mode=None) as pg:
                pid = _check_server(pg)
...
def test_start_failure_log(caplog):
    ...
    """
    with tempfile.TemporaryDirectory() as tmpdir:
        with embedded_postgres.get_server(tmpdir) as _:
            pass
...
def test_no_conflict():
    """ test we can start two servers on two different datadirs with no conflict (eg port conflict)
    """
    pid1 = None
    pid2 = None
    try:
        with tempfile.TemporaryDirectory() as tmpdir1, tempfile.TemporaryDirectory() as tmpdir2:
            with embedded_postgres.get_server(tmpdir1) as pg1, embedded_postgres.get_server(tmpdir2) as pg2:
...
def _reuse_deleted_datadir(prefix: str):
    ...
    try:
        for _ in range(num_tries):
            ...
            postmaster = embedded_postgres.utils.PostmasterInfo.read_from_pgdata(pgdata)
